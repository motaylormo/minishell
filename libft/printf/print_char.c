...
static int	to_unicode(wchar_t c)
{
	unsigned int	octet;
	...
	else if (c <= 0x7FF)
	{
		...
		return (write(1, &octet, 2));
	}
	else if (c <= 0xFFFF)
	{
		octet = (((c & 0x3f) << 16) |
				(((c >> 6) & 0x3f) << 8) | (c >> 12)) | 0x8080e0;
		return (write(1, &octet, 3));
	}
	else if (c <= 0x10FFFF)
	{
		octet = (((((c & 0x3f) << 24) | (((c >> 6) & 0x3f) << 16))
				| (((c >> 12) & 0x3f) << 8)) | (c >> 18)) | 0x808080f0;
		return (write(1, &octet, 4));
	}
	return (-1);
}

static int	inner(wchar_t c, t_fwplt fwplt)
{
	int			char_count;
	t_padding	padding;
...
int			pf_print_char(va_list ap, t_fwplt fwplt)
{
	if (fwplt.type == 'C' || fwplt.length == l)
		return (inner(va_arg(ap, wchar_t), fwplt));
	else if (fwplt.type == 'c')
		return (inner(va_arg(ap, unsigned int), fwplt));
	return (-1);
}
