...
static int	inner(long double num, t_fwplt fwplt)
{
	int			char_count;
	char		*num_str;
	char		*head;
	...
	char_count += write(1, num_str, ft_strlen(num_str));
...
int			pf_print_floating_point(va_list ap, t_fwplt fwplt)
{
	...
	if (arg == INFINITY || arg == -INFINITY)
		return ((arg == -INFINITY) ? write(1, "-inf", 4) : write(1, "inf", 3));
	return (inner(arg, fwplt));
}
