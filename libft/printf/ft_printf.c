...
static int	get_str_seg(const char **str)
{
	int	char_count;
	int	i;
...
static int (*g_print_table[])(va_list, t_fwplt) =
{
	['s'] = pf_print_string,
	['c'] = pf_print_char,
	['C'] = pf_print_char,
...
static int	parse(const char *str, va_list ap)
{
	int		char_count;
	t_fwplt	fwplt;
	...
	while (1)
	{
		char_count += get_str_seg(&str);
		if (*str == '\0')
			break ;
		fwplt = pf_get_fwplt(&str, fwplt, ap);
		if (fwplt.type != -1)
			char_count += g_print_table[fwplt.type](ap, fwplt);
	}
	return (char_count);
}
